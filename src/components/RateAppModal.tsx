...
export default function RateAppModal({ visible, onSubmit, onSkip }: RateAppModalProps) {
  ...
  const handleSubmit = async () => {
    try {
      // Analytics: app rating submitted
      trackEvent('app_rating_submitted', { rating });
  ...
  const handleSkip = async () => {
    try {
      // Analytics: app rating skipped
      trackEvent('app_rating_skipped');
      await onSkip?.();
    } catch (_) {}
  };
    ...
    <Modal visible={visible} transparent animationType="fade">
      <View style={[styles.overlay, { backgroundColor: colors.scrim || 'rgba(0,0,0,0.45)' }]}>
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}> 
          <Text style={[styles.title, { color: colors.textPrimary, textAlign: isRTL ? 'right' : 'left' }]}>{t('rateApp.title')}</Text>
          <Text style={[styles.subtitle, { color: colors.textSecondary, textAlign: isRTL ? 'right' : 'left' }]}>{t('rateApp.subtitle')}</Text>

          <View style={styles.starsRow}>
            {[1,2,3,4,5].map((s) => (
              <TouchableOpacity key={s} onPress={() => setRating(s)}>
                <Text style={[styles.star, { color: colors.accentBlue }]}>{rating >= s ? '★' : '☆'}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <TextInput
            ...
            placeholder={t('rateApp.placeholder')}
...
const styles = StyleSheet.create({
  ...
  starsRow: { flexDirection: 'row', gap: 8, justifyContent: 'center', marginVertical: 12 },
  star: { fontSize: 28 },
  input: { marginTop: 8, borderWidth: 1, borderRadius: 8, padding: 10, textAlignVertical: 'top' },
