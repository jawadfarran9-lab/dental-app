...
function SendingOverlay({ visible, pct }: { visible: boolean; pct: number | null }) {
  ...
  useEffect(() => {
    if (pct != null) {
      progress.value = withTiming(Math.min(1, Math.max(0, pct / 100)), { duration: 250 });
    }
  }, [pct, progress]);
  ...
  const ringProps = useAnimatedProps(() => ({
    strokeDashoffset: RING_C * (1 - (determinate ? progress.value : 0.28)),
  }));
  const spinStyle = useAnimatedStyle(() => ({ transform: [{ rotate: `${spin.value * 360}deg` }] }));

  if (!visible) return null;
  const phase = pct == null ? 'Preparing…' : pct < 100 ? 'Compressing video…' : 'Uploading…';
  ...
  return (
    <View style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
      ...
      <View style={{ width: 148, paddingVertical: 18, paddingHorizontal: 16, borderRadius: 24, backgroundColor: 'rgba(15,23,42,0.82)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.09)', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.35, shadowRadius: 18, elevation: 10 }}>
        ...
        <Text style={{ color: 'rgba(255,255,255,0.9)', marginTop: 10, fontSize: 12.5, fontWeight: '600', letterSpacing: 0.3 }}>{phase}</Text>
      </View>
    </View>
  );
}
    ...
    <GestureHandlerRootView style={styles.root}>
      ...
      {previewUri && (
        <View ref={compositeRef} style={styles.previewOverlay}>
                ...
                {(['free', '1:1', '4:5', '9:16'] as const).map((r) => {
                  ...
                  return (
                    <Pressable
                      ...
                      hitSlop={6}
                      style={{ paddingHorizontal: 14, paddingVertical: 7, marginHorizontal: 5, borderRadius: 16, backgroundColor: on ? '#1E6FD9' : 'rgba(255,255,255,0.12)', borderWidth: 1, borderColor: on ? '#1E6FD9' : 'rgba(255,255,255,0.25)' }}
                    >
          ...
          <SendingOverlay visible={sending} pct={sendPct} />
        </View>
      )}
    </GestureHandlerRootView>
  );
