...
export default function ArchivedPatientsScreen() {
  useClinicGuard();
    ...
    try {
      ...
      const list: PatientRow[] = snap.docs
        ...
        .map((d) => {
          ...
          let archivedAtMs: number | undefined;
          if (raw && typeof raw.toMillis === 'function') archivedAtMs = raw.toMillis();
          else if (raw && typeof raw.seconds === 'number') archivedAtMs = raw.seconds * 1000;
          else if (typeof raw === 'number') archivedAtMs = raw;
          return {
            id: d.id,
      ...
      list.sort((a, b) => {
        if (a.archivedAtMs === undefined && b.archivedAtMs === undefined) return 0;
        if (a.archivedAtMs === undefined) return 1;
        if (b.archivedAtMs === undefined) return -1;
        return b.archivedAtMs - a.archivedAtMs;
      });
      setPatients(list);
    } catch (err) {
      console.error('[archived-patients] fetch error', err);
    } finally {
      setLoading(false);
    }
  ...
  const restorePatient = useCallback(
    async (patientId: string) => {
      if (!clinicId) return;
      try {
        const pRef = doc(db, 'clinics', clinicId, 'patients', patientId);
        await updateDoc(pRef, { archived: false, archivedAt: null });
        await loadPatients();
      } catch (err: any) {
        console.error('[archived-patients] restore error', err);
        Alert.alert('Error', err?.message || 'Failed to restore patient');
      }
    },
    [clinicId, loadPatients]
  );

  const handleRestorePress = (p: PatientRow) => {
    Alert.alert(
      'Restore patient',
      `Restore ${p.name}? They will be able to log in again.`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Restore', style: 'default', onPress: () => restorePatient(p.id) },
      ]
    );
  };
          ...
          >
            ...
            {patients.length === 0 ? (
              ...
              >
                ...
                >
                  <Ionicons name="trash-outline" size={26} color="#F43F5E" />
                </View>
                <Text style={[styles.emptyTitle, { color: textPrimary }]}>
                  No deleted patients
                </Text>
                <Text style={[styles.emptySub, { color: textMuted }]}>
                  Patients you remove will show up here.
                </Text>
              </View>
            ) : (
              patients.map((p) => {
