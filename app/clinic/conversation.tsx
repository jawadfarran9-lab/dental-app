...
export default function ClinicConversationScreen() {
  ...
  useEffect(() => {
    ...
    for (const id of ids) {
      const ref = doc(db, `patients/${patientId as string}/appointments/${id}`);
      const unsub = onSnapshot(
        ref,
        (snap) => {
        ...
        (err) => console.warn('[clinic/conversation] appt live sub', err),
      );
      unsubs.push(unsub);
    }
    return () => { for (const u of unsubs) u(); };
  }, [summaryApptIdsKey, patientId]);
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (wasLast && clinicId) {
                const preview = prev ? (prev.type === 'image' ? 'Photo' : prev.type === 'audio' ? '🎤 Voice message' : prev.type === 'video' ? '🎬 Video' : prev.text) : '';
                try {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <MediaViewerModal
        ...
        canDelete={(p) => messages.find((x) => x.id === p.msgId)?.from === 'clinic'}
