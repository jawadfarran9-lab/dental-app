...
import { ActivityIndicator, Alert, BackHandler, InteractionManager, SafeAreaView, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function UpgradeScreen() {
  ...
  const [storedAIPro, setStoredAIPro] = useState(false);
  ...
  const [clinicNameFromFirestore, setClinicNameFromFirestore] = useState('');
    ...
    const loadCurrentPlan = async () => {
      try {
        ...
        if (clinicSnap.exists()) {
          ...
          setClinicNameFromFirestore((data.clinicName as string) || '');
        }
      } catch (error) {
  ...
  const computedPrice = useMemo(() => {
    ...
    const final = discountPercent > 0
      ...
      : base;
    return { base, final, period: plan.period, name: plan.name };
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      const pendingKeys: [string, string][] = [
        ...
        ['pendingClinicName', clinicNameFromFirestore || ''],
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.cardContainer}>
          {/* ── Section Title ── */}
          <Text style={[styles.sectionTitle, { color: textPrimary }]}>Choose Your Plan</Text>
                ...
                >
                  ...
                  >
                    ...
                    <View style={styles.planHeader}>
                      <Text style={[styles.planName, { color: textPrimary }]}>{plan.name}</Text>
                      {plan.badge && (
                    ...
                    <View style={styles.priceSection}>
                      <View style={styles.priceRow}>
                        <Text
                          style={[styles.planPrice, { color: textPrimary }]}
                          numberOfLines={1}
          ...
          >
            ...
            <Text style={[styles.aiProDesc, { color: isDark ? 'rgba(255,255,255,0.52)' : muted }]}>
            ...
            <View style={styles.aiProFeatures}>
              {[
                ...
                <View key={idx} style={styles.aiProFeatureItem}>
                  ...
                  <Text style={[styles.aiProFeatureText, { color: isDark ? 'rgba(255,255,255,0.74)' : textPrimary }]}>{feat}</Text>
                </View>
              ))}
            </View>
          </GlassCard>
          ...
          >
            <Text style={[styles.paymentSectionTitle, { color: textPrimary }]}>
            ...
            <View style={styles.paymentMethodsGrid}>
              ...
              >
                ...
                <Text style={[styles.paymentMethodLabelText, { color: textPrimary }]}>{t('payment.card', 'Card')}</Text>
                {selectedPaymentMethod === 'card' && (
              ...
              >
                ...
                <Text style={[styles.paymentMethodLabelText, { color: textPrimary }]}>{t('payment.applePay', 'Apple Pay')}</Text>
                {selectedPaymentMethod === 'apple-pay' && (
              ...
              >
                ...
                <Text style={[styles.paymentMethodLabelText, { color: textPrimary }]}>{t('payment.paypal', 'PayPal')}</Text>
                {selectedPaymentMethod === 'paypal' && (
              ...
              >
                ...
                <Text style={[styles.paymentMethodLabelText, { color: textPrimary }]}>{t('payment.googlePay', 'Google Pay')}</Text>
                {selectedPaymentMethod === 'google-pay' && (
          ...
          {selectedPaymentMethod === 'card' && (
            ...
            >
              ...
              <TextInput
                style={[styles.cardInput, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.9)', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', color: textPrimary }]}
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TextInput
                  style={[styles.cardInput, { flex: 1, backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.9)', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', color: textPrimary }]}
              ...
              {computedPrice && (
                <View style={styles.totalRow}>
                  <Text style={[styles.totalLabel, { color: isDark ? 'rgba(255,255,255,0.52)' : muted }]}>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
              <Ionicons name="pricetag-outline" size={18} color={muted} style={{ marginRight: 8 }} />
              <Text style={[styles.paymentSectionTitle, { color: textPrimary, marginBottom: 0 }]}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 8 }}>
              <TextInput
                style={[styles.couponInput, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.9)', borderColor: appliedCoupon ? '#10b981' : isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', color: textPrimary, borderWidth: appliedCoupon ? 2 : 1 }]}
                placeholder={t('payment.enterCoupon', 'Enter coupon code...')}
                placeholderTextColor={isDark ? 'rgba(255,255,255,0.35)' : '#9CA3AF'}
                value={couponCode}
                onChangeText={setCouponCode}
                editable={!saving && !appliedCoupon}
                maxLength={20}
              />
              {!appliedCoupon ? (
                <TouchableOpacity
          ...
          >
            {saving ? (
              ...
              <Text style={[styles.confirmBtnText, { color: canConfirm ? '#fff' : muted }]}>
                {computedPrice
